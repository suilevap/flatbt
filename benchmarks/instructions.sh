...
SCENARIOS=${SCENARIOS:-"select8 patrol guard soldier villager"}
# CARGO_FLAGS=--no-default-features measures a FlatBT revision from before the catalog.
cargo build --release --quiet ${CARGO_FLAGS:-}
...
for s in $SCENARIOS; do
  ...
  echo "|---|--:|--:|"
  for l in $LIBS; do
    # Libraries that cannot express the scenario have no entry for it.
    "$BIN" ticks "$l" "$s" 1 >/dev/null 2>&1 || continue
