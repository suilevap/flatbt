...
use arena::world::{ARENA, Ammo, Arena, Cover, Health, Player, Speed, track_arena};
use bevy::prelude::*;
...
fn main() {
    ...
    App::new()
        ...
        // The gather, the trees, the components their decisions become, and the
        // systems that act on them -- all of `ai`, in one line.
        .add_plugins(arena::ai::plugin)
        ...
        .add_systems(
            ...
            (
                ...
                (recover_in_cover, adjust_population).after(ActionSystems),
...
///
/// Clearing `cover` is what sends them back through the asking branch, so
/// `find_cover` answers more than once per agent over a session. It is the
/// game's field to clear, the same way it was the game's to fill.
fn recover_in_cover(time: Res<Time>, mut hiding: Query<(&Transform, &mut Health, &mut Fighter)>) {
    for (transform, mut health, mut fighter) in hiding.iter_mut() {
        let Some(&spot) = fighter.cover.answered() else {
            continue;
        };
        if transform.translation.truncate().distance(spot) > 10.0 {
            continue;
        }
        health.0 = (health.0 + 25.0 * time.delta_secs()).min(100.0);
        if health.0 >= 80.0 {
            fighter.bypass_change_detection().cover.clear();
        }
    }
}
