...
//! why the tick has no `Commands` at all: at the cost of a real tree, a
//! per-entity `ParallelCommands` scope costs more than the tick it guards, and
//! batching it back is a complication a blackboard component does not need --
//! a tree writes its decision and a later system carries it out.
//!
//! ```sh
