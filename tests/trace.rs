...
fn a_tree_that_failed_outright_is_still_traced() {
    ...
    state.set_trace(true);
    assert_eq!(
        update(&tree, &mut state, &mut 0, EntryMode::Evaluate),
        NodeResult::Failure
    );
...
fn a_node_entered_several_times_lists_each_outcome() {
    ...
    let mut state: BtState<_, _> = BtState::new(&tree);
    state.set_trace(true);
    let _ = update(&tree, &mut state, &mut 9, EntryMode::Evaluate);
    assert_eq!(
        format!("{:#}", state.trace()),
...
#[test]
fn a_state_without_a_trace_says_so() {
    let tree = tree();
    let mut state = BtState::new(&tree);
    let _ = update(&tree, &mut state, &mut 0, EntryMode::Evaluate);
    assert_eq!(state.trace().to_string(), "trace off");
}
...
fn a_slot_driver_keeps_its_own_log_with_a_limit() {
    ...
    let _ = update_slot_traced(&tree, &mut slot, &mut 0, EntryMode::Evaluate, Some(&log));
    assert!(log.overflowed());
    assert_eq!(log.calls().count(), 2);
    assert!(
        format!(
            "{:#}",
            trace::<u32, &str, _>(&tree, slot.as_ref(), Some(&log))
        )
